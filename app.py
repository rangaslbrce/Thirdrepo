...
if choose == "Descriptive Analytics":
    ...
    with col1:               # To display the header text using css style
        ...
        st.markdown('<p class="font">Food Demand Forecasting using Machine Learning & Stastical Analysis</p>', unsafe_allow_html=True)
    ...
    with col8:
        ...
        base_sum=df.groupby(["category"]).checkout_price.agg("mean")
    ...
    with col9:
        ...
        base_sum = df.groupby(['cuisine']).agg({'num_orders':'mean'})
        center = base_sum.index
...
# Returns the absolute value of data frame num_orders
    numeric_df=df.apply(pd.to.numeric,errors='coerce')
...
# Applying log transformation on the target feature
    ...
    st.subheader("srihari More - y22mc2005")    
